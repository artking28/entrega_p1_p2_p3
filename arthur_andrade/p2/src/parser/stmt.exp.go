...
func (parser *MantisParser) ParseExpression(sub bool) (IExp, error) {
	token := parser.Get(0)
	if token == nil {
	...
	for i := 0; token != nil; i++ {
		...
		if token.Kind == models.SPACE || token.Kind == models.TAB {
			...
			ret.AddTerm(e)
		} else if token.Kind == models.UNDERLINE {
			parser.Consume(1)
			token = parser.Get(0)
			return nil, nil
		} else if token.Kind == models.NIL {
			return nil, errors.New("nil values are not supported")
		} else if token.Kind == models.TRUE || token.Kind == models.FALSE {
			if ret != nil && !lastWasSig {
				return nil, errors.New("consecutive numbers")
			}
			lastWasSig = false
			...
			if TypeOf(ret, parser) != "bool" && TypeOf(ret, parser) != "unknown" {
				return nil, utils.GetMismatchedTypesErr(parser.Filename, "number", "boolean", parser.At())
			}
			ret.AddTerm(NewBExp(string(token.Value) == "true"))
			...
			if token.Kind == models.ID {
				variable := parser.VariablesNames[string(token.Value)]
				if variable == nil {
					return nil, errors.New("unknown id '" + string(token.Value) + "'")
				}
				ret.AddTerm(NewIdExp(variable.Id))
			} else {
				ret.AddTerm(NewVExp(uint64(token.Value[0])))
			}
